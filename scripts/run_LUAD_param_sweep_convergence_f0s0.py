#!/usr/bin/env python3
"""LUAD 参数收敛 sweep（单 fold_0 + seed_0，P1 best.pt 复用已有）。

覆盖基础数据：
  1. 窗口收敛点（sweep-type=window_grid 16 组稀疏网格，L∈{0.50,0.53,0.56,0.59,0.62} × U∈{0.65,0.68,0.72,0.80}）
  2. A 收敛点（sweep-type=A，A∈[0.5,0.6,0.7,0.8,0.9,0.95,0.99,0.995,0.999] 9 组，窗口固定 0.59-0.68（LUAD 原冠军））
  6. 激活窗口数据量化：每 run summary.json 存 activation_epoch = 首个 w_now < 0.1 的 epoch（训练日志 parser 自动挖），最终 report 输出「窗口激活后 Δ_C_index = C_post − C_pre」

策略：
  - fold=0，seed=0（简单），P1 best.pt 直接复用 LUAD 主链路已产出的（censored_stage_survival_full_upgrade/.../phase1/fold_0_seed0/best.pt）
  - skip_existing，断电断点续跑
  - --only-report 打印排名 + Δ退化检查 + A/窗口二阶差分找「收敛拐点」
"""
...
OUT_ROOT = PROJECT_ROOT / "outputs" / "censored_stage_survival_LUAD_param_sweep_f0s0"

PRETRAIN_P1_BEST = (
    PROJECT_ROOT / "outputs" / "censored_stage_survival_full_upgrade"
    / "wsi=UNI1024_tilefix256_rna=hallmark50_omics" / "win0.59-0.65_A0.99_K50"
    / "phase1" / "fold_0_seed0" / "best.pt"
)
...
def _parse_activation_epoch(run_log: Path) -> tuple[int | None, float | None, float | None]:
    """从 run.log 挖窗口激活 epoch（首个 w_now < 0.1 的 epoch），并读该 epoch 前后的 (test_c_before, test_c_after)。
    返回 (activation_epoch, test_c_before_max, test_c_after_max)"""
    ...
    for line in txt.splitlines():
        ...
        try:
            ...
            wf = float(w)
            tc = line.split("test_c=")[1].split()[0] if "test_c=" in line else None
...
def _find_inflection_point_1d(xs_sorted: list[float], ys_sorted: list[float]) -> float | None:
    """ys = f(xs)，找二阶差分绝对值最大的点（拐点）。返回对应 x。"""
    if len(ys_sorted) < 4:
        return None
...
def summary_report_A() -> str:
    ...
    p1_c = _p1_c()
    lines.append(f"\n{'='*110}")
    lines.append(f"[LUAD A 收敛]  fold=0 seed=0   窗口={FIXED_WINDOW_FOR_A}  P1_baseline={p1_c:.4f}")
...
def summary_report_window() -> str:
    ...
    p1_c = _p1_c()
    lines.append(f"\n{'='*130}")
    lines.append(f"[LUAD 窗口收敛]  fold=0 seed=0   A=0.99   P1_baseline={p1_c:.4f}")
    ...
    if rows_s:
        ...
        lines.append(f"📉 各 L 下最优 U（收敛建议）：" + ", ".join(f"L={L:.2f}→U={max_U[L][1]:.2f}(C={max_U[L][0]:.3f})" for L in Ls if max_U[L][0] > 0))
    return "\n".join(lines)
...
def main() -> None:
    ...
    if not PRETRAIN_P1_BEST.exists():
        raise SystemExit(f"[FATAL] P1 best.pt 不存在：{PRETRAIN_P1_BEST}（需要先跑 LUAD 主链路 P1）")
    ...
    print(f"[INFO] LUAD 参数收敛 sweep 计划 {len(plans)} runs   (fold={SWEEP_FOLD} seed={SWEEP_SEED})")
    print(f"[INFO]   P1 best.pt（复用）：{PRETRAIN_P1_BEST}")
    for (st_dir, specs) in sweeps_in_order:
