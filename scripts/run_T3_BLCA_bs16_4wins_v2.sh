...
PRETRAIN_EXP="BLCA_pretrain_bs16_ep50"

for s in ${SEEDS}; do
  for k in ${FOLDS}; do
    STAGE1_DIR="/root/autodl-tmp/R2wsp/outputs/${PRETRAIN_EXP}/${COHORT}/seed${s}/fold${k}/stage1"
    ...
    for WIN in ${WINS}; do
      EXP_NAME="BLCA_${WIN}_bs16_ep50_v2"
      STAGE2_DIR="/root/autodl-tmp/R2wsp/outputs/${EXP_NAME}/${COHORT}/seed${s}/fold${k}/stage2"
      ...
      BASE_CMD=(python /root/autodl-tmp/R2wsp/scripts/train_censored_stage_survival.py
        --device cuda
        --stage 2
        --cohort_hint ${COHORT}
        --wsi_feature_source uni1024
      ...
      if [ "${WIN}" = "fixed_off" ]; then
      ...
      elif [ "${WIN}" = "param_best_mix" ]; then
        "${BASE_CMD[@]}" \
          --loss_window_policy param \
          --loss_window_metric val_c_index_ema \
          --loss_window_center_mode best_mix \
          --loss_window_best_mix_alpha 0.5 \
        ...
        "${BASE_CMD[@]}" \
          --loss_window_policy onek \
          --loss_window_K 0.25 \
          --loss_window_eps 0.05 \
          || echo "[FAIL stage2] ${COHORT} s=${s} k=${k} win=${WIN}"
      else
        "${BASE_CMD[@]}" \
          --loss_window_policy twok \
          --loss_window_K_left 0.35 \
          --loss_window_K_right 0.20 \
          --loss_window_eps 0.05 \
          || echo "[FAIL stage2] ${COHORT} s=${s} k=${k} win=${WIN}"
      fi
