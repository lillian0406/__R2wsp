#!/usr/bin/env python3
"""两阶段 pilot 调度器：k=3 seed=0，3 折 × 2 阶段 × 2 个升级实验。
升级内容：
  Exp: UNI 1024 tilefix256 (1024d) + rna_mode=omics (HALLMARK50)
与主 scheduler 严格对齐，仅改动：
  (1) BASE_ARGS: wsi_feature_source / rna_mode=omics + gtf + batch_size 保守 4
  (2) N_FOLDS=3, SEEDS=[0] (pilot)
  (3) OUT_ROOT 加 FEAT_TAG 做目录隔离，避免覆盖原 5 折结果
  (4) WSI 输入维度 512d → 1024d，不复用旧 PLIP Phase-1 权重，每个实验独立 Phase-1 初始化
"""
...
OUT_ROOT = PROJECT_ROOT / "outputs" / "censored_stage_survival_pilot_upgrade"

# ---- 窗口参数（严格沿用主 scheduler 最优窗口 0.59-0.65 A=0.99）----
WINDOW_LOWER = 0.59
WINDOW_UPPER = 0.65
WINDOW_K = 50.0
WINDOW_A = 0.99
...
def build_phase1_plan(exp: ExpSpec, fold: int, seed: int) -> RunPlan:
    ...
    out_dir = OUT_ROOT / exp.feat_tag / _WIN_TAG / EVAL_KIND / "phase1" / f"fold_{fold}_seed{seed}"
...
def build_phase2_plan(exp: ExpSpec, fold: int, seed: int, eval_kind: str = EVAL_KIND) -> RunPlan:
    split_dir = PHASE2_ROOT / f"fold_{fold}" / eval_kind
    pretrain = OUT_ROOT / exp.feat_tag / _WIN_TAG / EVAL_KIND / "phase1" / f"fold_{fold}_seed{seed}" / "best.pt"
    ...
    return RunPlan(tag=f"[P2]{exp.feat_tag[:22]}.._f{fold}_s{seed}", cmd=cmd, out_dir=out_dir, log_file=log_file)
...
def summary_report() -> str:
    ...
    lines.append("=" * 96)
    lines.append(f"PILOT UPGRADE 汇总报告  ({t0})   窗口={_WIN_TAG}  k={N_FOLDS}  seeds={SEEDS}  eval={EVAL_KIND}")
    lines.append(f"Baseline (旧 PLIP 512d + RNA vec, k3 seed0 eval_with_cens 3-fold mean) = 0.5809")
    lines.append("")
    ...
    lines.append(f"{'排名':>4s}  {'实验 feat_tag':<64s}  {'N':>2s}   {'mean±std':<16s}  vsBaseline    fold c-index (best_ep)")
    ranked = sorted(rows, key=lambda r: (st.mean(r[1]) if r[1] else -1.0), reverse=True)
    for rank, (tag, vals, eps) in enumerate(ranked, 1):
        if not vals:
            lines.append(f"{rank:>4d}  {tag:<64s}  {'0':>2s}   {'未完成':<16s}")
            continue
        m = st.mean(vals)
        sd = st.pstdev(vals) if len(vals) > 1 else 0.0
        gain_pp = (m - 0.5809) * 100.0
        cells = "  ".join(f"{v:.4f}(e{ep})" for v, ep in zip(vals, eps))
        lines.append(f"{rank:>4d}  {tag:<64s}  {len(vals):>2d}   {m:.4f}±{sd:.4f}    {gain_pp:+.2f} pp    {cells}")
    lines.append("")
    lines.append(f"增益阈值：≥ +2.00 pp → 判定有效，补 full k5 × 3seeds × 2eval = 30 次；< +2.00 pp → 诊断。")
    return "\n".join(lines)
...
def main() -> None:
    ...
    ap.add_argument("--only-exp", default=None, help="只跑指定 feat_tag 前缀匹配的 experiment")
    args = ap.parse_args()
        ...
        for f in range(N_FOLDS):
            for s in SEEDS:
                ...
                if args.only in ("phase2", "all"):
                    plans.append(build_phase2_plan(exp, f, s))
    ...
    print(f"[INFO] 计划 {len(plans)} runs  ({len(exps)} exps × {N_FOLDS} folds × {len(SEEDS)} seeds × ({'P1+' if args.only in ('phase1','all') else ''}{'P2' if args.only in ('phase2','all') else ''}))")
    print(f"[INFO] _WIN_TAG = {_WIN_TAG}   EVAL_KIND = {EVAL_KIND}")
    ...
    rep = summary_report()
    print("\n" + rep)
    out_txt = OUT_ROOT / f"summary_k3_seed0_{_WIN_TAG}_{EVAL_KIND}.txt"
